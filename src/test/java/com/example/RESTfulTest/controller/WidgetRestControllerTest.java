...
class WidgetRestControllerTest {
    ...
    void testPutWidgetNoFound() throws Exception{
        Widget widgetToPut  = new Widget(1l, "Widget Name", "Description", 1);
        doReturn(Optional.empty()).when(service).findById(1L);
    ...
    void testPutWidgetFound() throws Exception {
        ...
        Widget widgetToReturnSave = new Widget(1L, "Widget puted", "This is my puted widget", 2);
        ...
        mockMvc.perform(put("/rest/widget/{id}", 1l)
                .contentType(MediaType.APPLICATION_JSON)
                .header(HttpHeaders.IF_MATCH, 1)
                .content(asJsonString(widgetToReturnSave)))
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.version", is(2)))
                .andExpect(jsonPath("$.name", is("Widget puted")))
                .andExpect(jsonPath("$.description", is("This is my puted widget")));
